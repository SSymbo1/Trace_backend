...
import edu.hrbu.trace_backend_business.entity.po.Supplier;
import edu.hrbu.trace_backend_business.mapper.ProductMapper;
import edu.hrbu.trace_backend_business.mapper.ProductRecordMapper;
import edu.hrbu.trace_backend_business.mapper.SupplierMapper;
...
public class SubjectServiceImpl implements SubjectService {
    ...
    public Result requestProductPaged(ProductQuery query) {
        IPage<edu.hrbu.trace_backend_business.entity.po.Product> page = new Page<>(query.getCurrentPage(), query.getPageSize());
        Map<String, Object> condition = getProductSearchCondition(query);
        IPage<edu.hrbu.trace_backend_business.entity.po.Product> productPage = productMapper.selectProductByCondition(
                page, condition
        );
    ...
    public Result requestProductAdd(Product product) {
        ...
        edu.hrbu.trace_backend_business.entity.po.Product insertProduct = edu.hrbu.trace_backend_business.entity.po.Product.builder()
                .cid(product.getCid())
    ...
    public Result requestEditProduct(Product product) {
        DateTime localTime = new DateTime(DateTime.now());
        Integer currentAccountId = Integer.valueOf(JwtUtil.parseJWT(OnlineContext.getCurrent()).getSubject());
        edu.hrbu.trace_backend_business.entity.po.Product updateProduct = edu.hrbu.trace_backend_business.entity.po.Product.builder()
                .pid(product.getPid())
                .cid(product.getCid())
                .eid(product.getEnterprise())
                .name(product.getName())
    ...
    public Result requestProductRecordPaged(ProductQuery query) {
        IPage<edu.hrbu.trace_backend_business.entity.po.Product> page = new Page<>(query.getCurrentPage(), query.getPageSize());
        Map<String, Object> condition = getProductSearchCondition(query);
        IPage<edu.hrbu.trace_backend_business.entity.po.Product> productPage = productRecordMapper.selectProductRecordProcessByCondition(
                page, condition
        );
        productPage.getRecords().forEach(product -> product.setPhoto(photoPath + product.getPhoto()));
        return Result

...
import edu.hrbu.trace_backend_job.entity.dto.system.*;
import edu.hrbu.trace_backend_job.entity.dto.system.Account;
import edu.hrbu.trace_backend_job.entity.dto.system.Role;
...
import edu.hrbu.trace_backend_job.service.SystemService;
import edu.hrbu.trace_backend_job.util.AesUtil;
...
public class SystemServiceImpl implements SystemService {
    ...
    public Result requestAccountAdd(Account account) {
        ...
        QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Account> existAccountWrapper = new QueryWrapper<>();
        existAccountWrapper.eq("username", account.getUsername());
        edu.hrbu.trace_backend_job.entity.po.Role role = roleMapper.selectById(account.getRole());
        ...
        edu.hrbu.trace_backend_job.entity.po.Account insertAccount = edu.hrbu.trace_backend_job.entity.po.Account.builder()
    ...
    public Result requestAccountStatueSet(AccountStatue statue) {
        edu.hrbu.trace_backend_job.entity.po.Account updateAccountStatue = null;
        if (statue.getDel().equals(1)) {
            updateAccountStatue = edu.hrbu.trace_backend_job.entity.po.Account.builder()
                    .aid(statue.getAid())
                    .del(statue.getDel())
                    .ban(1).build();
        } else {
        ...
        if (statue.getBan().equals(0)) {
            edu.hrbu.trace_backend_job.entity.po.Account account = accountMapper.selectById(statue.getAid());
            QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Role> roleQueryWrapper = new QueryWrapper<>();
    ...
    public Result requestAccountEdit(Account account) {
        ...
        edu.hrbu.trace_backend_job.entity.po.Account updateAccount = edu.hrbu.trace_backend_job.entity.po.Account.builder()
                .aid(account.getAid())
                .username(account.getUsername())
                .password(AesUtil.encryptHex(account.getPassword()))
        ...
        AccountInfo updateAccountInfo = AccountInfo.builder()
                ...
                .address(account.getAddress())
                .zipCode(account.getZipCode()).build();
        edu.hrbu.trace_backend_job.entity.po.Account exist = accountMapper.selectById(account.getAid());
        if (exist.getUsername().equals(account.getUsername())) {
            ...
            QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Account> existWrapper = new QueryWrapper<>();
    ...
    public Result requestDisableAllAccount(Able able) {
        ...
        QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Account> queryWrapper = new QueryWrapper<>();
        queryWrapper
                .ne("rid", 1).
                and(ban -> ban.ne("ban", 1));
        edu.hrbu.trace_backend_job.entity.po.Account updateBan = edu.hrbu.trace_backend_job.entity.po.Account.builder()
                .ban(1).build();
    ...
    public Result requestEnableAllAccount(Able able) {
        ...
        List<edu.hrbu.trace_backend_job.entity.po.Account> accounts = accountMapper.selectList(queryWrapper);
        if (!accounts.isEmpty()) {
            for (edu.hrbu.trace_backend_job.entity.po.Account account : accounts) {
                edu.hrbu.trace_backend_job.entity.po.Role role = roleMapper.selectById(account.getRid());
                if (role.getBan().equals(1)) {
    ...
    public Result requestEnterpriseInfoPaged(EnterpriseQuery query) {
        IPage<edu.hrbu.trace_backend_job.entity.po.Enterprise> page = new Page<>(
    ...
    @Override
    public Result requestEnterpriseAdd(edu.hrbu.trace_backend_job.entity.dto.system.Enterprise enterprise) {
        Integer currentAccountId = Integer.valueOf(JwtUtil.parseJWT(OnlineContext.getCurrent()).getSubject());
        QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Enterprise> existWrapper = new QueryWrapper<>();
        ...
        edu.hrbu.trace_backend_job.entity.po.Enterprise insertEnterprise = edu.hrbu.trace_backend_job.entity.po.Enterprise.builder()
    ...
    @Override
    public Result requestEnterpriseEdit(edu.hrbu.trace_backend_job.entity.dto.system.Enterprise enterprise) {
        edu.hrbu.trace_backend_job.entity.po.Enterprise updateEnterprise = edu.hrbu.trace_backend_job.entity.po.Enterprise.builder()
                ...
                .ilk(enterprise.getIlk())
                .zipCode(enterprise.getZipCode()).build();
        edu.hrbu.trace_backend_job.entity.po.Enterprise exist = enterpriseMapper.selectById(enterprise.getEid());
    ...
    public Result requestRoleStatueSet(RoleStatue statue) {
        edu.hrbu.trace_backend_job.entity.po.Role updateRoleStatue = null;
        if (statue.getDel().equals(1)) {
            updateRoleStatue = edu.hrbu.trace_backend_job.entity.po.Role.builder()
                    .rid(statue.getRid())
                    .del(statue.getDel())
                    .ban(1).build();
        } else {
        ...
        if (updateRoleStatue.getBan().equals(1)) {
            QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Account> accountQueryWrapper = new QueryWrapper<>();
            accountQueryWrapper.eq("rid", updateRoleStatue.getRid());
            List<edu.hrbu.trace_backend_job.entity.po.Account> accounts = accountMapper.selectList(accountQueryWrapper);
            if (!accounts.isEmpty()) {
    ...
    public Result requestRoleInfoPaged(RoleQuery query) {
        IPage<edu.hrbu.trace_backend_job.entity.po.Role> page = new Page<>(query.getCurrentPage(), query.getPageSize());
        Map<String, Object> condition = getRoleInfoSearchCondition(query);
        IPage<edu.hrbu.trace_backend_job.entity.po.Role> data = roleMapper.selectRoleByCondition(page, condition);
    ...
    public Result requestRoleEdit(Role role) {
        ...
        edu.hrbu.trace_backend_job.entity.po.Role updateRole = edu.hrbu.trace_backend_job.entity.po.Role.builder()
                .rid(role.getRid())
                .name(role.getName())
                .memo(role.getMemo()).build();
        edu.hrbu.trace_backend_job.entity.po.Role exist = roleMapper.selectById(role.getRid());
        if (exist.getName().equals(role.getName())) {
            ...
            QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Role> existWrapper = new QueryWrapper<>();
    ...
    public Result requestRoleAdd(Role role) {
        ...
        edu.hrbu.trace_backend_job.entity.po.Role addRole = edu.hrbu.trace_backend_job.entity.po.Role.builder()
                .name(role.getName())
    ...
    public Result requestEnableAllRole(Able able) {
        ...
        QueryWrapper<edu.hrbu.trace_backend_job.entity.po.Role> queryWrapper = new QueryWrapper<>();
        queryWrapper
                .ne("rid", 1)
                .and(ban -> ban.ne("ban", 0))
                .and(del -> del.ne("del", 1));
